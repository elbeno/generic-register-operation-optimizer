...
namespace groov {
namespace detail {
template <typename Group, typename Mask, typename Register>
auto read(Register const &r) -> async::sender auto {
    using bus_t = typename Group::bus_t;
    return bus_t::template read<Register::name, Mask::value>(get_address(r));
}
...
constexpr auto read(read_spec<Group, Paths> const &s) -> async::sender auto {
    ...
    return []<typename... Rs, typename... Ms>(stdx::tuple<Rs...>,
                                              stdx::tuple<Ms...>) {
        return async::when_all(detail::read<Group, Ms>(Rs{})...) |
