...
template <pathlike Path, typename Value> struct value_path : Path {
    ...
    constexpr auto resolve(P) const {
        if constexpr (can_resolve<Path, P>) {
            constexpr auto leftover_path = groov::resolve(Path{}, P{});
            if constexpr (leftover_path.empty()) {
                if constexpr (stdx::tuple_size_v<value_t> == 1) {
                    return get<0>(value);
                } else {
                    return value;
                }
            } else {
                return detail::value_path_build_helper(leftover_path, value);
            }
        } else {
            return resolution_t<Path, P>{};
        }
    }

    // template <pathlike P> constexpr auto resolve2([[maybe_unused]] P p) const
    // {
