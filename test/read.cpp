...
TEST_CASE("read a register by path", "[read]") {
    using namespace groov::literals;
    data[0] = 0xa5a5'a5a5u;
    auto s = read(grp / "reg0"_r);
    auto r = get<0>(*(s | async::sync_wait()));
    CHECK(r["reg0"_r] == data[0]);
}

TEST_CASE("read a register by type", "[read]") {
    using namespace groov::literals;
    data[0] = 0xa5a5'a5a5u;
    auto s = read(grp / R0{});
    auto r = get<0>(*(s | async::sync_wait()));
...
TEST_CASE("read a register (indexed)", "[read]") {
    using namespace groov::literals;
    data[0] = 0;
    data[1] = 0xa5a5'a5a5u;
    auto s = read(grpi / "reg[1]"_r);
    auto r = get<0>(*(s | async::sync_wait()));
    CHECK(r["reg[1]"_r] == data[1]);
}

TEST_CASE("read a register by type (compile-time indexed)", "[read]") {
    using namespace groov::literals;
    using namespace stdx::literals;
    data[0] = 0;
    data[1] = 0xa5a5'a5a5u;
    auto s = read(grpi / Idx{}[1_c]);
    auto r = get<0>(*(s | async::sync_wait()));
    CHECK(r["reg"_r] == data[1]);
    CHECK(r["reg[1]"_r] == data[1]);
}
...
//     using namespace groov::literals;
//     data[0] = 0b111'1010'0u;
//     data[1] = 0b111'0101'1u;
...
//     CHECK(r["reg0"_f] == data[0]);
//     CHECK(r["reg1"_f] == data[1]);
//     CHECK(r["reg0.field0"_f] == 0);
//     CHECK(r["reg0.field1"_f] == 0b1010u);
//     CHECK(r["reg1.field0"_f] == 1u);
//     CHECK(r["reg1.field1"_f] == 0b0101u);
...
//     using namespace groov::literals;
//     [[maybe_unused]] auto r = sync_read(grp("reg0.field0"_f,
//     "reg0.field2"_f)); CHECK(bus::last_mask == 0b111'0000'1u);
// }

// TEST_CASE("read is pipeable", "[read]") {
//     using namespace groov::literals;
//     data[0] = 0xa5a5'a5a5u;
//     auto r = async::just(grp / "reg0"_r) | groov::read() |
//     async::sync_wait(); CHECK(get<0>(*r)["reg0"_r] == data[0]);
// }
...
//     CHECK(get<0>(*r)["reg0"_r] == data[0]);
// }

// namespace {
// template <auto Addr> std::uint32_t data_ct{};

// struct ct_address_bus {
//     template <stdx::ct_string, auto Mask, typename T>
//     static auto read(T) -> async::sender auto {
//         return async::just_result_of([=] { return data_ct<T::value>; });
//     }
...
//     using namespace groov::literals;
//     data_ct<42> = 0xa5a5'a5a5u;
//     auto r = sync_read(g / "reg"_r);
//     CHECK(r["reg"_r] == data_ct<42>);
// }
...
// using R4 =
//     groov::reg<"reg4", std::uint32_t, &data3, groov::w::replace, FB0,
//     FB1_WO>;
...
// TEST_CASE("read a field in a register containing WO fields which won't be
// read "
